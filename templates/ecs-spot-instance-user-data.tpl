...
file = /var/log/spot-termination.log
log_group_name = /var/log/spot/termination
log_stream_name = {cluster}/{container_instance_id}
datetime_format = %b %d %H:%M:%S
...
Content-Type: text/x-shellscript; charset="us-ascii"
#!/bin/bash
cat > /usr/local/bin/cloudwatch-logs-starter.sh <<- EOF
	#!/bin/bash
	set -ex
...
aws --region ap-southeast-2 logs put-retention-policy --log-group-name /var/log/spot/termination --retention-in-days 30
...
cat <<- EOF > /usr/local/bin/spot-instance-termination-notice-handler.sh
	...
	until curl -s http://localhost:51678/v1/metadata > /dev/null
	do
	  sleep 1
	done
	...
	while sleep 5; do
	  if [ -z "\$(curl -Isf http://169.254.169.254/latest/meta-data/spot/termination-time)" ]; then
	    ...
	    echo "Termination Notification Received..."
	    ...
	    cistatus=\`aws --region ap-southeast-2 ecs update-container-instances-state --cluster "\$ECS_CLUSTER" --container-instances "\$CONTAINER_INSTANCE" --status DRAINING --output json\`
	    echo \$cistatus
	  fi
	done
EOF
...
Content-Type: text/x-shellscript; charset="us-ascii"
#!/bin/bash
cat <<-EOF > /etc/systemd/system/spot_instance_termination_notice_handler.service
[Unit]
Description=Spot Instance Termination Notice Handler
[Service]
WorkingDirectory=/usr/local/bin
ExecStart=/usr/local/bin/spot-instance-termination-notice-handler.sh
Restart=on-failure
EOF
systemctl enable spot_instance_termination_notice_handler.service
systemctl start spot_instance_termination_notice_handler.service
systemctl daemon-reload
...
cat <<-EOF > /etc/rsyslog.d/31-spot-notification.conf
# Log spot notification generated log messages to file
:syslogtag, isequal, "spot-instance-termination-notice-handler.sh:" /var/log/spot-termination.log

# comment out the following line to allow spot notification messages through.
# Doing so means you will also get spot notification messages in /var/log/syslog
& stop
EOF
